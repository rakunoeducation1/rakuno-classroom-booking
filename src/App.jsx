...
// 3号大教室的 id。来自上方 ROOMS 里的 3号大教室。
const BLOCKED_ROOM_ID = 'atelier-1f'

// 18:00 及以后不可预约。
const BLOCKED_AFTER_MINUTES = 18 * 60
...
function dayLabel(date) {
  ...
  return `${date.getMonth() + 1}/${date.getDate()}（${w[date.getDay()]}）`
}

function keyOf(date, roomId, slot) {
  return `${date}__${roomId}__${slot}`
}

// 把 "18:00-19:00" 取出开始时间，转成分钟数。
...
// 核心限制：3号大教室，周四/周五，18:00及以后不可预约。
function isBlockedSlot(date, roomId, slot) {
  const weekday = parseDateKey(date).getDay() // 0日 1月 2火 3水 4木 5金 6土

  return (
    roomId === BLOCKED_ROOM_ID &&
    (weekday === 4 || weekday === 5) &&
    slotStartMinutes(slot) >= BLOCKED_AFTER_MINUTES
  )
}
...
export default function App() {
  ...
  const visibleRooms = useMemo(
    () => ROOMS.filter(r => selectedRoom === 'all' || r.id === selectedRoom),
    [selectedRoom]
  )

  const bookingsMap = useMemo(
    () => new Map(bookings.map(b => [keyOf(b.date, b.room_id, b.slot), b])),
    [bookings]
  )
  ...
  const stats = useMemo(() => {
    const total = ROOMS.reduce((sum, room) => {
      const roomBookableSlots = TIME_SLOTS.filter(slot => !isBlockedSlot(selectedDate, room.id, slot)).length
      return sum + roomBookableSlots
    }, 0)

    const used = bookings.filter(
      b => b.date === selectedDate && !isBlockedSlot(b.date, b.room_id, b.slot)
    ).length
  ...
  const dailyBookings = useMemo(() => {
    ...
    return bookings
      ...
      .filter(b =>
        !q ||
        [b.name, b.purpose, b.memo, b.slot, ROOMS.find(r => r.id === b.room_id)?.name]
          .join(' ')
          .toLowerCase()
          .includes(q)
      )
  ...
  function openSlot(date, roomId, slot) {
    ...
    // 新增预约时拦截。
    if (isBlockedSlot(date, roomId, slot)) {
      alert('3号大教室每周四、周五18:00以后不可预约。')
      return
    }
  ...
  async function saveBooking() {
    ...
    // 保存时再次拦截，防止绕过按钮直接提交。
    if (modal.mode === 'new' && isBlockedSlot(modal.date, modal.roomId, modal.slot)) {
      alert('3号大教室每周四、周五18:00以后不可预约。')
      return
    }
      ...
      <section className="toolbar">
        ...
        <div className="filters">
          ...
          <input
            ...
            placeholder="搜索学生、老师、课程、备注"
          />

          <button onClick={fetchBookings} disabled={!supabase || loading}>
            {loading ? '刷新中' : '刷新'}
          </button>
            ...
            <thead>
              <tr>
                ...
                {visibleRooms.map(room => (
                  <th key={room.id}>
                    {room.name}
                    <small> 容纳{room.capacity}人｜{room.note}</small>
                  </th>
                ))}
              </tr>
            </thead>
              ...
              {TIME_SLOTS.map(slot => (
                <tr key={slot}>
                  ...
                  {visibleRooms.map(room => {
                    ...
                    const blocked = isBlockedSlot(selectedDate, room.id, slot)

                    return (
                      <td key={room.id}>
                        {b ? (
                          <button className="booked" onClick={() => openSlot(selectedDate, room.id, slot)}>
                            <span>{b.purpose}</span>
                            <b>{b.name}</b>
                            {b.memo && <em>{b.memo}</em>}
                          </button>
                          ...
                          <button className="empty" onClick={() => openSlot(selectedDate, room.id, slot)}>
                            + 预约
                          </button>
                        )}
                      </td>
            ...
            dailyBookings.map(b => {
              ...
              return (
                <div className="detail" key={b.id}>
                  <div>
                    <b>{b.slot}</b>
                    <button onClick={() => openSlot(b.date, b.room_id, b.slot)}>编辑</button>
                    <button onClick={() => deleteBooking(b.id)}>删除</button>
                  </div>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <label>
              ...
              <textarea
                ...
                placeholder="老师、人数、注意事项等"
              />
            </label>

            <div className="modalActions">
              {modal.mode === 'edit' && (
                <button className="danger" onClick={() => deleteBooking(modal.booking.id)}>
                  删除
                </button>
              )}
