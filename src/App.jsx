...
export default function App() {
  ...
  const stats = useMemo(() => {
    const total = ROOMS.reduce((sum, room) => {
      const bookableCount = TIME_SLOTS.filter(slot => !isBlockedSlot(selectedDate, room.id, slot)).length
      return sum + bookableCount
    }, 0)
  ...
  function openSlot(date, roomId, slot) {
    ...
    if (!existing && isBlockedSlot(date, roomId, slot)) {
      ...
      {modal && (
        <div className="modalBackdrop" onClick={() => setModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <p>
              {modal.date}｜{ROOMS.find(r => r.id === modal.roomId)?.name}｜{modal.slot}
            </p>
